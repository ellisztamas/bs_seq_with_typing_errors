...
meth_states = pd.concat([
    ml.methylation_state(chloroplast, ab_errors, return_probabilities=True, hard_calls=True).assign(type='Chloroplast'),
    ml.methylation_state(genes, ab_errors, return_probabilities=True, hard_calls=True).assign(type='Genes'),
    ml.methylation_state(rddm, ab_errors, return_probabilities=True, hard_calls=True).assign(type='RdDM TEs'),
    ml.methylation_state(cmt2, ab_errors, return_probabilities=True, hard_calls=True).assign(type='CMT2 TEs')
])
